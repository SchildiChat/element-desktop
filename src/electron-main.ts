...
import { app, BrowserWindow, Menu, autoUpdater, protocol, dialog, nativeTheme, Input, Event } from "electron";
...
async function setupGlobals(): Promise<void> {
    ...
    global.trayConfig = {
        ...
        brand: global.vectorConfig.brand || "SchildiChat",
    };
...
// SC: electron requestSingleInstanceLock does not work on macos-mas builds, only use it on non mas builds
if (process.mas !== true) {
...
app.on("second-instance", (ev, commandLine, workingDirectory) => {
    // SC: workaround for Linux system theme change
...
app.setAppUserModelId("com.squirrel.schildichat-desktop.SchildiChat");
